...
from phase0_questions import QUESTION_BANK, CATEGORY_META, CATEGORY_WEIGHTS, SCALE_LABELS
from phase0_logic import (
...
def render_phase_0():
    ...
    st.write("This phase tests whether you are personally and financially suited for franchising before you get pulled into a deal.")
    st.info("Answer as you are today. Not as you hope to be later.")
    ...
    if st.button("Calculate Reality Check ", type="primary"):
        ...
        st.session_state["phase_0_answers"] = answers
        st.session_state["phase_0_score"] = total_score
        st.session_state["phase_0_verdict"] = verdict
        st.session_state["phase_0_category_scores"] = category_scores

        st.markdown("---")
        st.subheader("Reality Check Result")

        st.markdown(
            f"""
            <div style="padding:20px; border-radius:14px; border:1px solid #ddd; background:#fafafa;">
                <div style="font-size:13px; color:#666; margin-bottom:6px;">Ownership Readiness</div>
        ...
        st.markdown("### What You Still Do Not Know")
        st.write("- How to close the gaps that matter most")
        st.write("- Whether a specific concept can be structured safely")
        st.write("- What to negotiate before signing anything")
        st.write("- Whether you should proceed, pause, or walk away")
        ...
        st.markdown(
            ...
            <div style="padding:18px; border-radius:14px; background:#f6f6f6; border:1px solid #ddd; margin-top:10px;">
                ...
                <div style="font-size:16px; margin-bottom:8px;">Move to Concept Validation to test whether the concept itself is worth pursuing.</div>
                <div style="font-size:14px; color:#555;">Good personal readiness does not automatically make a concept viable.</div>
            </div>
            """,
            unsafe_allow_html=True
        )
